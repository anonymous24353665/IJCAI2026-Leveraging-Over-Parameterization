...
# =========================
# Classe custom per rete shallow
# =========================
...
# =========================
# Funzione modulare per training shallow
# =========================
def run_training_shallow(
    dataset_name: str,
    config_path_ini: str,
    ...
    """
    Funzione modulare per allenare reti shallow con ricerca binaria dei parametri.

    Parametri:
        dataset_name: nome del dataset
        config_path_ini: percorso file INI di configurazione
        config_path_yaml: percorso file YAML di configurazione
        hidden_layers_dim: lista dimensioni hidden layer
        min_increment, max_increment, steps_limit: parametri ricerca binaria
        rs_loss_bool: se calcolare la rs_loss
        skip_binary_search: saltare la ricerca binaria
        debug: abilitare profiling cProfile
    """
    # Carica configurazione
    config = load_config(config_path_ini)

    # Costruisci tuple architettura: [(784, h, 10), ...]
    architectures = [(784, x, 10) for x in hidden_layers_dim]

    # Inizializza ricerca binaria dei parametri
